...
local switcher_keys = {}
for i = 1, 9 do
	switcher_keys[tostring(i)] = { app = "", run = "" }
end
...
qlaunch._fake_keys = {
	{
		...
		nil,
		{
			description = "Select app (run or rise if selected already) by key",
			group = "Action",
...
local function build_state_indicator(style)
	...
	function widg:draw(_, cr, width, height)
		...
		for i = 1, n do
			cr:set_source(
				color(
					data.state[i].focused and style.color.main
						or data.state[i].urgent and style.color.urgent
						or data.state[i].minimized and style.color.gray
						or style.color.icon
				)
			)
...
local function build_switcher(keys, style)
	...
	function widg:update(store, idb)
		...
		for key, data in pairs(store) do
			...
			if data.icon ~= "" then
				icon = flex.service.dfparser.lookup_icon(data.icon, style)
			end
		...
		if self.items[key] then
			...
			if self.selected == key then
				...
				for _, v in ipairs(mod) do
					modcheck = modcheck and awful.util.table.hasitem(qlaunch.forcemod, v)
				end
...
function qlaunch:init(args, style)
	...
	self.icon_db = flex.service.dfparser.icon_list(style.parser)
...
function qlaunch:show()
	...
	modtip:set_pack(
		"Quick launch",
		self.tip,
		self.style.keytip.column,
		self.style.keytip.geometry,
		self.style.keytip.exit and function()
			self:hide()
		end
	)
end
...
function qlaunch:set_new_app(key, c)
	...
	if c then
		...
		local note =
			modutil.table.merge({ text = string.format("%s binded with '%s'", c.class, key) }, self.style.notify)
		flex.float.notify:show(note)
	else
