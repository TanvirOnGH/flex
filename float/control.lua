...
control._fake_keys = {
	{
		...
		nil,
		{
			description = "Select move/resize step",
			group = "Mode",
...
local function control_off_screen(window)
	...
	if newg.width > wa.width then
		window:geometry({ width = wa.width, x = wa.x })
	end
	if newg.height > wa.height then
		window:geometry({ height = wa.height, y = wa.y })
	end
...
function control:resize(is_shrinking)
	...
	-- validate new size
	if newg.height and newg.height <= 0 or newg.width and newg.width < 0 then
		return
	end
...
function control:show()
	...
	if not self.wibox.visible then
		...
		local is_floating = client.focus
			and (client.focus.floating or layout.name == "floating")
		...
		modtip:set_pack("Floating window", self.tip, self.style.keytip.column, self.style.keytip.geometry, function()
			self:hide()
		end)
	end
end
...
function control:set_keys(keys, layout)
	...
	if keys then
		...
		if layout ~= "all" then
			self.keys.all = awful.util.table.join(self.keys.control, self.keys.action)
		end
	end
