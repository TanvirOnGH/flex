...
function placement.no_offscreen(object, gap, area)
	...
	if gap then
		area = placement.add_gap(area, gap)
	end
...
local function control_off_screen(window, workarea)
	...
	if wg.width > wa.width then
		window:geometry({ width = wa.width, x = wa.x })
	end
	if wg.height > wa.height then
...
local function centered_base(is_h, is_v)
	return function(object, gap, area)
		...
		if is_h then
			new_geometry.x = area.x + (area.width - geometry.width) / 2 - object.border_width
		end
		if is_v then
			new_geometry.y = area.y + (area.height - geometry.height) / 2 - object.border_width
		end
...
placement.centered = setmetatable({}, {
	__call = function(_, ...)
		return centered_base(true, true)(...)
	end,
})
